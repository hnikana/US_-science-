...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar  4 20:05:11 2020
...
def multi_constructor(each_census):
    error_ = []
    df_collector= []
...
start = time.time()  
pool = mp.Pool(4)
results = pool.map(multi_constructor ,[each_census for each_census in state_census])
    #results = pool.map(partial(constructor ,Census_period_purchase_, Demo_ , Sales_) ,[household  for household in each])
pool.close()


Data =  pd.concat([result[1] for result in results])
Error = pd.concat([result[2] for result in results])


Data.to_csv(path+'data_state_3/data_{}.csv'.format(state_name), index= False)
Error.to_csv(path+'data_state_3/error_{}.csv'.format(state_name) , index = False ) 

Error.to_pickle(path+'data_state_3/error_{}.pkl'.format(state_name))
Data.to_pickle(path+'data_state_3/data_{}.pkl'.format(state_name))
 
duratioon =  time.time() -  start
