...
RF = RandomForestClassifier(max_depth = 30 , class_weight={0:1,1:20})
...
AUC_curve(RF , X_test, y_test , y_pred , y_score , title = 'Random_forest_AUC_Curve_1_30' , path = '')
plot_importance (RF.feature_importances_ , 20 , df_final.columns , title = 'Randome_Forest_feature_importance_1_30 ' ,  path = './'  )
...
def objective_cross(params):
    ...
    test_metric = precision_recall_fscore_support(y_test , y_pred , average = 'binary')
...
best_cross = fmin(objective_cross,space,algo=tpe.suggest,trials = bayes_trail, max_evals=400)

def tune_result (trials) :
    df = pd.DataFrame()
    df['loss'] = trials.losses()
    df['train_metric'] = [trials.trials[i]['result']['train_score']for i in range(400)]
    df['test_metric'] = [trials.trials[i]['result']['test_score']for i in range(400)]
    df['params'] = [trials.trials[i]['result']['model'].get_params(deep = True) for i in range(400)]
    return df
...
(training_inputs, testing_inputs, training_classes,
  testing_classes) = train_test_split(X, y, test_size=0.20, random_state=1)
...
#                **para['fit_params'])
        loss = 1 - f1_score(self.y_train , clf.predict(self.x_train))

        pred = clf.predict(self.x_test)
        test_score  = f1_score(self.y_test, pred )

       
       
        return {'loss': loss, 'status': STATUS_OK , 'model':clf , 'test_score':test_score}
...
params = {"objective":"reg:logistic",'colsample_bytree': 0.3, 'feval':f1_eval , 'learning_rate': 0.1,
                'max_depth': 5, 'alpha': 10}



CV = xgb.cv(dtrain=data_dmatrix, params=params, nfold=3,
                    num_boost_round=50,early_stopping_rounds=10, as_pandas=True, seed=123)


model = xgb.XGBClassifier()
kfold = KFold(n_splits=10, random_state=7)
results_2 = cross_val_score(model, X_train, y_train, cv=2 ,scoring='f1_macro' )
